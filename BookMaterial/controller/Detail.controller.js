		...
		return BaseController.extend("iae.controller.Detail", {
			...
			onSave: function() {
				...
				if( !data.EINDT || data.EINDT === "" || 
					...
					var dialog		=	new sap.m.Dialog({
						title: that._oResourceBundle.getText("ERROR_DATA"),
						type: 'Message',
						content: new sap.m.Text({ text: that._oResourceBundle.getText("FIELDS") + fields + that._oResourceBundle.getText("REQUIRED") }),
	 	...
	 	// 		//	Set HTML Content
			...
			confirmPrestation: function( event ){
				...
				this._NewPoste.getBinding("items").filter( new sap.ui.model.Filter( "NAME1" , sap.ui.model.FilterOperator.Contains , "" ) );
